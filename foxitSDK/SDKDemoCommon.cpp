...
FS_RESULT FSDK_Document::addAnnot(PageHandle page, TextPageHandle textPage, MatrixHandle matrix, Platform::String^ word) {
	...
	Inherited_PDFFunction tmp_class;
	...
	*/
	...
	while (isMatchTemp) {
		...
		for (int count = 0; count < num; count++) {
			...
			///////////////avoid condition like this word = "for" and we find out "information"
			int x = (rect.left + rect.right) / 2 ;
			int y = (rect.bottom + rect.top) / 2 ;
			String^ tmp_word = tmp_class.GetWordFromLocation(textPage, matrix, x, y, 0);
...
//type = 1 means: position in render image
//type = 0 means: coordinate in page ,no need to use matrix
Platform::String^ Inherited_PDFFunction::GetWordFromLocation(TextPageHandle itextpage, MatrixHandle imatrix, float x, float y, int type)
{
	FS_RESULT ret;
	//load textpage
	/*FSPDF_TEXTPAGE textPage = NULL;
	ret = FSPDF_TextPage_Load((FSCRT_PAGE)(page.pointer), &textPage); //Load text page
	if (ret != FSCRT_ERRCODE_SUCCESS)
	{
		Platform::String^ word = " 00";
		return word;
	}*/
	FSPDF_TEXTPAGE textPage = (FSPDF_TEXTPAGE)itextpage.pointer;
	...
	if (type == 1)
	{
		FSCRT_MATRIX mt = *(FSCRT_MATRIX *)imatrix.pointer;

		//get xx and yy from x and y 
		//*(xx,yy) is coordinate in page             (x,y) is position in render image
		xx = (x - mt.e)*mt.d - mt.c*(y - mt.f);
		xx /= (mt.a*mt.d - mt.c*mt.b);
		yy = (y - mt.f)*mt.a - (x - mt.e)*mt.b;
		yy /= (mt.a*mt.d - mt.c*mt.b);
	}
	else
	{
...
FS_RESULT Inherited_PDFFunction::SearchInCurrentPage(Platform::String^ searchText, TextPageHandle textPageH)
{
	std::string s;
	...
	textPage = (FSPDF_TEXTPAGE)textPageH.pointer;

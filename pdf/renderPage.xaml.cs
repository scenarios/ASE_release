...
namespace pdf
{
	...
	public struct pageInfo
    {
        ...
        public int renderState;//renderState = 1: have completed; renderState = 0: is rendering; renderState = -1: no render;
    }
    ...
    public sealed partial class renderPage : Page
    {
        ...
        private async void initPdfGrid(int pageNum)
        {
            ...
            if (!visiblePage.ContainsKey(iPageIndex))
            {
                ...
                else
                {
                    ...
                    if (0 == result)
                    {
                        ...
                        result = m_SDKDocument.LoadTextPage(pageptr.hPage, out pageptr.hTextPage);
                        if (result != 0)
                        {
                            pageptr.hTextPage.pointer = 0;
                            return 0;
                        ...
                        pageptr.renderState = -1;
                        visiblePage.Add(iPageIndex, pageptr);
                        //m_iCurPageIndex = iPageIndex;
                        //GetPageInfo();
                    }
                }
                SelectWordsInCurrentPageFromWordList(iPageIndex);
            }
            else
            {
                result = 0;
        ...
        private void unLoadPage(int iPageIndex)
        {
            if (visiblePage.ContainsKey(iPageIndex))
            {
                if(visiblePage[iPageIndex].renderState != 0)
                {
        ...
        ///////////////////////////////////////////////////////////////get words which exist in current page and wordlist database
        List<String> m_sWordListHaveRead=new List<string>() ;
        
        /////////////////i will put word showing up in page which have loaded and parsed in the string
        /////////////////the string format is "true word\0word\0word\0" same with the result return from GetAllWordFromWordBook function in DB
        ///////////////////the string contain repeat word ,using it after deleting repeating word;
        
        private void SelectWordsInCurrentPageFromWordList(int iPageIndex)
        {
            if (visiblePage.ContainsKey(iPageIndex))
            {
                pageInfo pageptr = visiblePage[iPageIndex];
                int result;
                if (pageptr.hTextPage.pointer == 0)
                {
                    result = m_SDKDocument.LoadTextPage(pageptr.hPage, out pageptr.hTextPage);
                    if (result != 0)
                    {
                    ...
                    visiblePage[iPageIndex] = pageptr;
                }
                //search wordlist in current page
                foreach (wordInfo wInfo in wordInfoList)
                {
                    
                    result = m_PDFFunction.SearchInCurrentPage(wInfo.word, pageptr.hTextPage);
                    if (result == 0)
                    {
                        if(!m_sWordListHaveRead.Contains(wInfo.word))
                        {
                            m_sWordListHaveRead.Add(wInfo.word);
                        }
        ...
        */
        ...
        id_manager id_manager_ = new id_manager();
        ...
        private async void GetEndLocation(object sender, PointerRoutedEventArgs e)
        {
            ...
            if (m_mousestate == true)
            {
                ...
                if (m_BeginLocation.X == m_EndLocation.X && m_BeginLocation.Y == m_EndLocation.Y)
                {
                    ...
                    pageInfo pageptr = visiblePage[currentPage];
                    m_SDKDocument.LoadMatrix(pageptr.hPage, m_iStartX, m_iStartY, m_iRenderAreaSizeX, m_iRenderAreaSizeY, m_iRotation, out pageptr.hMatrix);
                    word = m_PDFFunction.GetWordFromLocation(pageptr.hTextPage, pageptr.hMatrix, (float)m_BeginLocation.X, (float)m_BeginLocation.Y, 1);
                    //popup definition of the word
                    //add the word wordinfolist
                    if (double_open==false&&word != "01" && word != "02" && word != "03" && word != "04")
                    {
                        double_open = true;
                        wordInfo wInfo;
                        bool[] testData = new bool[m_iPageCount];
                       ...
                       /* M.parti = PARTICIPANT.M_S;
                        M.method_m_s = METHOD_M_S.method_delete_wordbook;
                        M.in_message.wordbook_str = "default";
                        m_s_communication.message = M;
                        await m_s_communication.send_messageAsync();*/

                        int CurrentID = id_manager_.new_id();
                        String query = word;
                        M.parti = PARTICIPANT.P_M;
                        M.method_p_m = METHOD_P_M.method_click_event;
                        M.in_message.X = (int)(m_EndLocation.X);
                        M.in_message.Y = (int)(m_EndLocation.Y);
                        M.in_message.wordbook_str = "default";
                        M.in_message.word_str = query;
                        M.out_message.timer = id_manager_.timer_out;
                        M.in_message.id = CurrentID;
                        m_s_communication.message = M;
                        await m_s_communication.send_messageAsync();

                        await Task.Run(() => {
                            while (m_s_communication.message.out_message.closed_flag.Result != "closed") ;
                            double_open = false;
                            if (m_s_communication.message.out_message.op_success == true)
                            {
                                id_manager_.delete_id(CurrentID);
                            }
                            else
                            {
                                id_manager_.delete_id(CurrentID);
        ...
        {
            bool[] id = new bool[100];
            public bool[] timer_out = new bool[100];
            int length = 100;
            int front = 0, rear = 0;
            public id_manager()
            {
            ...
            private bool malloc_queue()
            {
                bool[] id1 = new bool[length + 100];
                bool[] timer_out1 = new bool[length + 100];
                if (id != null)
                {
                    ...
                    int i;
                    for (i = rear; (i + 1) % length != front; i++)
                    {
                        id1[i % (length + 100)] = id[i % length];
                        timer_out1[i % (length + 100)] = timer_out[i % length];
                    }
                    id = id1;
                    timer_out = timer_out1;
                    id1 = null;
                    timer_out1 = null;
                    front = i % (length + 100);
                    length += 100;
            ...
            {
                for (int i = rear; i % length != front; i++)
                {
                    if (i == Id)
                    {
                        id[i] = false;
        ...
        private void GetMoveLocation(object sender, PointerRoutedEventArgs e)
        {
            ...
            /*for (int i = 0; i < visiblePage.Count; i++) 
            {
                int index = visiblePage.ElementAt(i).Key;
                if (index >= firstVisibleIndex && index < nextInvisibleIndex)
                {
                    ;
                }
                else
        ...
        MESSAGE M1;
        SPECIFIC_COMMUNICATION communication = new SPECIFIC_COMMUNICATION();
        string[] Wordlist1;
        bool bgetword1 = false;
        private async Task ImportWordList1()
        {
            Wordlist1 = new string[m_sWordListHaveRead.Count];
            M1.parti = PARTICIPANT.M_S;
            M1.method_m_s = METHOD_M_S.method_lookup_word;
            M1.in_message.wordbook_str = "default";

            for (int i = 0; i < m_sWordListHaveRead.Count; i++)
            {
                M1.in_message.word_str = m_sWordListHaveRead[i];
                communication.message = M1;
                await communication.send_messageAsync();
                Wordlist1[i] = communication.message.out_message.info;
            }
            bgetword1 = true;
        }
